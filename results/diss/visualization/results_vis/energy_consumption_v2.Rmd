...
energy_cons_filter <- energy_cons %>%
  filter(Year == 2030, Sector %in% c("Q1", "Q2", "Q3", "Q4", "Q5", "sum_Q"))
...
energy_cons_filter <- energy_cons_filter %>%
  ...
  mutate(FF_hou = oil + gas + COL + CRU) %>%
  mutate(FF_car = oil + gas + COL + CRU + oil_car) %>%
  mutate(all_ele = ele_car + ele) %>%
  gather("oil", "gas", "COL", "CRU", "FF_hou", "ele", "total", "oil_car", "ele_car", "total_car", "all_ele", "FF_car", key = "Fuel", value = "Value") %>%
  mutate(Fuel = str_replace(Fuel, "FF_hou", "FF - housing")) %>%
  mutate(Fuel = str_replace(Fuel, "oil_car", "FF - transportation")) %>%  
  mutate(Fuel = str_replace(Fuel, "FF_car", "Fossil fuels")) %>%
  mutate(Fuel = str_replace(Fuel, "all_ele", "Electricity")) %>%
  mutate(Fuel = str_replace(Fuel, "total_car", "Total energy \nconsumption")) 
...
```{r}
energy_cons_diff_app <- energy_cons_diff %>%
  filter(Region != "EU+UK", Sector == "sum_Q", Fuel %in% c("Fossil fuels", "Electricity", "Total energy \nconsumption")) %>%
  pivot_wider(names_from = 'Scenario', values_from = 'Value')

write_csv(energy_cons_diff_app, 'energy_reg_app.csv')
``` 
...
energy_plot <- ggplot(filter(energy_cons_diff, 
                             ...
                             Sector %in% c("sum_Q"),
                             #Fuel %in% sel_ff
                             Fuel %in% c("Fossil fuels", "Electricity", "Total energy \nconsumption")
                             ), 
                      aes(x=Value, y=Fuel))
...
ggsave(width = 8, height = 5, filename = "ene_cons_reg2.png")
...
energy_cons_abs_app <- energy_cons_abs %>%
  filter(Region == "EU+UK", Sector == "sum_Q", Fuel %in% c("Fossil fuels", "Electricity", "Total energy \nconsumption")) %>%
  pivot_wider(names_from = 'Scenario', values_from = 'Value')

write_csv(energy_cons_abs_app, 'energy_eu_app.csv')
...
energy_plot2 +
  geom_point(aes(color = Scenario), size = 3, alpha = 0.5, height = 0.2) +
  facet_wrap(~Region) +
  #scale_fill_discrete(name = "Energy carrier") +
  scale_color_brewer(palette="Set1") +
...
```{r}
energy_cons_diff <- energy_cons_diff %>%
  mutate(Fuel = str_replace(Fuel, "FF \nTransportation", "FF \nTransport")) %>%
  mutate(Fuel = str_replace(Fuel, "FF - housing", "FF \nHousing"))
```

```{r}
energy_cons_hh_app <- energy_cons_diff %>%
  filter(Region %in% sel_reg_ene, Sector %in% c("Q1", "Q3", "Q5"), Fuel %in% c("FF \nHousing", "FF \nTransport", "Electricity", "Total energy \nconsumption")) %>%
  pivot_wider(names_from = 'Scenario', values_from = 'Value')

write_csv(energy_cons_hh_app, 'energy_hh_app.csv')
...
energy_plot3 <- ggplot(filter(energy_cons_diff, 
                              ...
                              Sector %in% c("Q1", "Q3", "Q5"),
                              #Fuel %in% sel_ff),
                              Fuel %in% c("Electricity", "Total energy \nconsumption", "FF \nHousing", "FF \nTransport")),
...
ggsave(width = 6, height = 6, filename = "ene_cons_hh2.png")
