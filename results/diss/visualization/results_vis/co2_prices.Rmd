...
all_prices_filter_1$Scenario <- gsub("VAT","CON", all_prices_filter_1$Scenario)
...
```{r}
inflation <-c(3.1,	2.6,	1.5,	0.6,	0.1,	0.2,	1.7,	1.9,	1.5)

inflation = 1 + (inflation/100)
total_inf = 1
for (i in inflation ) {
  total_inf = i*total_inf
}

all_prices_filter_1$Value_2020 = all_prices_filter_1$Value * total_inf

total_disc = 0.96^10

all_prices_filter_1$Value_2020_disc = all_prices_filter_1$Value_2020 * total_disc

```

```{r}
price_app <- all_prices_filter_1[-c(4,5)] %>%
  pivot_wider(names_from = 'Scenario', values_from = 'Value_2020_disc')

write_csv(price_app, 'price_app2.csv')
```

```{r}
all_prices_filter_1[-c(4,5)]
```


```{r}
plot_price <-ggplot(all_prices_filter_1, aes(x=Scenario, y = Value_2020_disc))
...
plot_price +
  ...
  geom_vline(xintercept=seq(1.5, length(unique(all_prices_filter_1$Scenario))-0.5, 1), 
             lwd=0.4, colour="black") +
  scale_y_continuous(name = "Price of t.eqCO2 [€ 2020]",
...
```
a new chunk by clicking the *Insert Chunk* button on the toolbar or by pressing *Ctrl+Alt+I*.
