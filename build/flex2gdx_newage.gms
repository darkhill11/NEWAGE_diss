...
*       Scale the data:
* ------ 18.07.2016: Scale and translate from Million USD 2011 to Billion Euros 2011
* Eurostat: 1 EUR = 1.3920 USD --> 1 USD = 7.183908E-4 EUR
* --> http://epp.eurostat.ec.europa.eu/tgm/table.do?tab=table&init=1&plugin=1&language=de&pcode=tec00033


scalar scalefactor /1e-3/;

scalefactor = scalefactor/1.3920;

vdep(r) = scalefactor * vdep(r);
vkb(r)  = scalefactor * vkb(r);
save(r) = scalefactor * save(r);
...
if (nd>0,
        ...
        vdep(r) = vdep(r)$round(vdep(r),nd);
        vkb(r)  = vkb(r)$round(vkb(r),nd);
        save(r) = save(r)$round(save(r),nd);
...
if (nd>0,
        execute_unload '%datadir%gsd_%nd%.gdx', r, f, g, i, vfm, vdfm, vifm, vxmd, vst, vtwr,
                rto, rtf, rtfd, rtfi, rtxs, rtms, esubd, esubva, esubm, etrae, eta, epsilon, evd, evi, evt, eco2d, eco2i, vdep, vkb, save;
else
