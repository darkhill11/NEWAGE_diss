...
*------ 05.10.2017 Schalter for sector-specific emission targets in Germany
...
SET
         ...
         EU28_eu(r)         EU28 (including the germany)
...
*-------13.10.2017 sec2cluster table, such that NEWAGE can assign i to sec and gen to sec
Parameter
    sec2cluster(sec,*);
...
sec2cluster("buildings","c") = 1;
sec2cluster("buildings","AGR") = 1;
sec2cluster("buildings","SER") = 1;
...
* ------ calibration for 2015 germany electricity generatio costs
         vafm_input0(i,*)       Extended parameter vafm value of aggregate firms inputs at market prices
         cap_input0(*)          Extended parameter kd0 capital earnings
         usk_input0(*)          Extended parameter uskd0 labor demand unskilled
         skl_input0(*)          Extended parameter skld0 labor demand skilled
...
vafm_input0(i,gen)  =  vafm_input(i,gen,"deu");
skl_input0(gen)     =  skl_input(gen,"deu");
usk_input0(gen)     =  usk_input(gen,"deu");
cap_input0(gen)     =  cap_input(gen,"deu");
...
*    co2em_sec(i,sec,r)  Sector specific CO2 emissions (aggregated over energy carriers) [Mio. tCO2]
    carblim_sec(sec,r)  Carbon emission limit for Germany
    carblim_sec0(sec,r) Carbon emission limit for Germany (benchmark)
...
* ------ LOAD
$LOAD    ressize
$LOAD    co2pfad_ets  co2pfad co2pfad_ets_eu co2pfad_ets_row co2pfad_ets_row_2dc co2pfad_nonets co2pfad_nonets_2dc co2pfad_DEU co2pfad_E3_eu co2pfad_cluni_nonets co2pfad_cluni_ets co2pfad_E4_eu co2pfad_regpush
$LOAD    co2pfad_REEEM_ets90 co2pfad_REEEM_ets95 co2pfad_REEEM_ALL90 co2pfad_REEEM_ALL95
$LOAD    URUN0  URSK0
...
* ------ 03.03.2017 switch for coal decomissioning in germany till 2030 
scalar
        noCOAL_DEU  switch for coal decomissioning in germany till 2035 / 0 /; // if (1) = coal decomissioning till 2035; else = NO coal decomissioning till 2030   
...
* ------ Quotas/Flags/Shares in electricty generation and ETS allowances
         ...
         row_notrade     Defines carbon regime in rest of world (without EU28) as regional permit price PCO2        / 0 / //*
...
* ------ Other
         ...
         DEU_CCS        Switch that enable (1) or desable (0) CCS in Germany                             / 1 /
         ...
         REEEM_run        switch just to make sure it is a reeem Scenario (enavi_e3)                     / 0 /
         REEEM_regpush    switch to turn on the regional push scenario for REEEM                         / 0 /
         ...
         REEEM_all_tiCap_95     switch to turn on tighter cap (95% red) for ALL Sectors                  / 0 /
...
display co2coefele, co2coefc, co2coefy;
...
pricetarget("col",r)             = 1 ;   // BAU on
pricetarget("gas",r)             = 1 ;   // BAU on
*pricetarget(i,"ROW") = 0;
* ------ 9.01.2015
...
aeei_cb_yr(r,i,cbt,cbf,"2011")   = aeei_cb(r,i,cbt,cbf) ;
aeei_co2_yr(r,cbt,cbf,"2011")    = aeei_co2(r,cbt,cbf) ;
...
* ------ 15.02.2016
loop(yr$after(yr),
         aeei_cb_yr(r,i,cbt,cbf,yr)$(c0_en(r,i,cbt,cbf) and yr2015(yr))          = aeei_cb_yr(r,i,cbt,cbf,yr-1) * (1-0.01 )**3 ;
         aeei_cb_yr(r,i,cbt,cbf,yr)$(c0_en(r,i,cbt,cbf) and after2015(yr))       = aeei_cb_yr(r,i,cbt,cbf,yr-1) * (1-0.01 )**5 ;
* ------ cars:
         aeei_co2_yr(r,ct,cf,yr)$(sum(i, cbco2i(r,i,ct,cf)) and yr2015(yr))   = aeei_co2_yr(r,ct,cf,yr-1) * (1-0.01 )**3 ;
         aeei_co2_yr(r,ct,cf,yr)$(sum(i, cbco2i(r,i,ct,cf)) and after2015(yr))= aeei_co2_yr(r,ct,cf,yr-1) * (1-0.01 )**5 ;
* ------ buildings:
         aeei_co2_yr(r,bt,bf,yr)$(sum(i, cbco2i(r,i,bt,bf)) and yr2015(yr))   = aeei_co2_yr(r,bt,bf,yr-1) * (1-0 )**3 ;
         aeei_co2_yr(r,bt,bf,yr)$(sum(i, cbco2i(r,i,bt,bf)) and after2015(yr))= aeei_co2_yr(r,bt,bf,yr-1) * (1-0 )**5 ;
);
...
bsd_yr(r,bt,bf,"2011")$bsd(r,bt,bf)  = 1 ;
ssd_yr(r,ct,cf,"2011")$ssd(r,ct,cf)  = 1 ;
bsd_yr(r,bt,bf,"2015")$bsd(r,bt,bf) = bsd_yr(r,bt,bf,"2011") * (1 - 3 * bsd_rate) ;
ssd_yr(r,ct,cf,"2015")$ssd(r,ct,cf) = ssd_yr(r,ct,cf,"2011") * (1 - 3 * ssd_rate) ;

loop(yr$after2015(yr),
         bsd_yr(r,bt,bf,yr)$bsd(r,bt,bf) = bsd_yr(r,bt,bf,yr-1)  - (5 * bsd_rate * bsd_yr(r,bt,bf,"2011")) ;
         ssd_yr(r,ct,cf,yr)$ssd(r,ct,cf) = ssd_yr(r,ct,cf,yr-1)  - (5 * ssd_rate * ssd_yr(r,ct,cf,"2011")) ;
         ssd_yr(r,ct,cf,yr)$(ssd_yr(r,ct,cf,yr) lt 0) = 0 ;
);

* ------ 5.01.2016
bsdyr(r,bt,bf,"2011")$(new(bt) and sum(i, bnd0(r,i,bt,bf))) = 1e-6 ;
...
*bsdyr0(r,bt,bf,"2011")$(new(bt) and sum(i, bnd0(r,i,bt,bf))) = sum(i, bnd0(r,i,bt,bf)) ;
...
bsdyr_init(r,bt,bf) = bsdyr(r,bt,bf,"2011") + bsdyr(r,bt,bf,"2015") + bsdyr(r,bt,bf,"2020") + bsdyr(r,bt,bf,"2025") + bsdyr(r,bt,bf,"2030") ;
...
***         PXTHOUSE07(r,bt,bf)$bsdyr(r,bt,bf,"2011")       ! Buildings stock from period 2011 (endowment)
...
* ------ Carbon regimes:
         i:PCO2(r)#(fe)$notrad(r)                 q:(co2em(fe,"final",r))        p:1e-6  fe.tl:          a:RA(r)
         i:PCO2W#(fe)$pco2w_r(r)$worldtrade       q:(co2em(fe,"final",r))        p:1e-6  fe.tl:          a:RA(r)
         i:PCO2W#(fe)$eu28(r)$eutrade             q:(co2em(fe,"final",r))        p:1e-6  fe.tl:          a:RA(r)
*         i:PCO2W#(fe)$deu(r)$detrade              q:(co2em(fe,"final",r) * aeei(fe,"c",r))        p:1e-6  fe.tl:          a:RA(r)
         i:PCO2W#(fe)$noneu28(r)$worldtrade2      q:(co2em(fe,"final",r))        p:1e-6  fe.tl:          a:RA(r)
         i:PCO2_NETS#(fe)$eu28(r)$netstrade       q:(co2em(fe,"final",r))        p:1e-6  fe.tl:
         i:PCO2_NETSr(r)#(fe)$eu28(r)$netstrade_r q:(co2em(fe,"final",r))        p:1e-6  fe.tl:             a:ra(r)    t:(carbon_tax(r)/co2coefc(fe,r))
...
*         i:PCO2W#(fe)$(DEU_sec and eu28(r) and eutrade)   q:(co2em(fe,"final",r) * aeei(fe,"c",r))        p:1e-6  fe.tl:  a:RA(r)
         i:PCO2_DEU("buildings")#(fe)$(DEU_sec and deu(r))        q:(co2em(fe,"final",r))    p:1e-6  fe.tl:  a:RA(r)
*--------16.10.2017 end

* ----- 26.01.2018
         i:PCO2(r)#(fe)$(deu(r) and detrade)                 q:(co2em(fe,"final",r))        p:1e-6  fe.tl:          a:RA(r)
...
*         e:PCO2W$carblim(r)$deu(r)$detrade               q:carblim_deu
...
* ------ 10.12.2014 (Erste Zeile Original)
$prod:Y(i,r)$nr(i,r)   s:0  vae(s):0.5  va(vae):1  e(vae):elast_ene(r)  nel(e):0.5  lqd(nel):2  oil(lqd):0  col(nel):0  gas(lqd):0
*$prod:Y(i,r)$nr(i,r)   s:0  vae(s):0.5  va(vae):1  e(vae):0.1  nel(e):0.5  lqd(nel):2  oil(lqd):0  col(nel):0  gas(lqd):0

* ------ 23.05.2014
         o:PY(i,r)               q:vom(i,r)             a:RA(r) t:ty(i,r)       a:RA(r)     N:GF_SUB(i,r)$(vom(i,r) AND gf_shr(i,r) AND gf_allow) M:(-1)$(vom(i,r) AND gf_shr(i,r) AND gf_allow)
...
* ------ Carbon regimes:
         i:PCO2(r)#(fe)$notrad(r)                                q:(co2em(fe,i,r))        p:1e-6  fe.tl:
         i:PCO2W#(fe)$pco2w_r(r)$worldtrade                      q:(co2em(fe,i,r))        p:1e-6  fe.tl:
         i:PCO2W#(fe)$eu28(r)$eutrade                            q:(co2em(fe,i,r))        p:1e-6  fe.tl:
*         i:PCO2W#(fe)$deu(r)$detrade                             q:(co2em(fe,i,r) * aeei(fe,i,r))        p:1e-6  fe.tl:
         i:PCO2W#(fe)$noneu28(r)$worldtrade2                     q:(co2em(fe,i,r))        p:1e-6  fe.tl:
         i:PCO2_ETS#(fe)$eu28(r)$ets(i)$etstrade                 q:(co2em(fe,i,r))        p:1e-6  fe.tl:     a:ra(r)     t:(carbon_tax(r)/co2coefy(fe,i,r))
         i:PCO2_NETS#(fe)$eu28(r)$(not ets(i))$netstrade         q:(co2em(fe,i,r))        p:1e-6  fe.tl:
         i:PCO2_NETSr(r)#(fe)$eu28(r)$(not ets(i))$netstrade_r   q:(co2em(fe,i,r))        p:1e-6  fe.tl:     a:ra(r)     t:(carbon_tax(r)/co2coefy(fe,i,r))
...
*    i:PCO2W#(fe)$(DEU_sec and eu28(r) and eutrade)                      q:(co2em(fe,i,r) * aeei(fe,i,r))    p:1e-6  fe.tl:
    i:PCO2_DEU(sec)#(fe)$(DEU_sec and deu(r) and sec2cluster(sec,i))    q:(co2em(fe,i,r))    p:1e-6  fe.tl:
*-------17.10.2017 end

* ----- 26.01.2018
         i:PCO2(r)#(fe)$(detrade and deu(r))                                q:(co2em(fe,i,r))        p:1e-6  fe.tl:
...
* ------ Carbon regimes:
         i:PCO2(r)#(fe)$notrad(r)                                q:(co2em(fe,gen,r))  p:1e-6
         i:PCO2W#(fe)$pco2w_r(r)$worldtrade                      q:(co2em(fe,gen,r))  p:1e-6
         i:PCO2W#(fe)$eu28(r)$eutrade                            q:(co2em(fe,gen,r))  p:1e-6
*         i:PCO2W#(fe)$deu(r)$detrade                             q:(co2em(fe,gen,r) * aeei_elex(gen,r))  p:1e-6
         i:PCO2W#(fe)$noneu28(r)$worldtrade2                     q:(co2em(fe,gen,r))  p:1e-6
         i:PCO2_ETS#(fe)$eu28(r)$etstrade                        q:(co2em(fe,gen,r))  p:1e-6     a:ra(r)     t:(carbon_tax(r)/co2coefele(fe,gen,r))
...
*    i:PCO2W#(fe)$(DEU_sec and eu28(r) and eutrade)           q:(co2em(fe,gen,r) * aeei_elex(gen,r))  p:1e-6
    i:PCO2_DEU(sec)#(fe)$(DEU_sec and deu(r) and sec2cluster(sec,gen))  q:(co2em(fe,gen,r))  p:1e-6
*-------13.10.2017 end
...
*-------13.10.2017 Sector-specific targets for Germany
    i:PCO2W#(fe)$(DEU_sec and eu28(r) and eutrade)                      q:(co2em(fe,gen,r))  p:1e-6
    i:PCO2_DEU(sec)#(fe)$(DEU_sec and deu(r) and sec2cluster(sec,gen))  q:(co2em(fe,gen,r))  p:1e-6
*-------13.10.2017 end
...
* ------ 17.06.2014 R_SUPPL doch nicht nötig wenn R_SUPPLY.L = 1
...
carblim_sec(sec,r)$deu(r) = sum(i$sec2cluster(sec,i),     sum(fe, co2em(fe,i,r)$nr(i,r)));

carblim_sec("buildings",r)$deu(r) = + sum(fe,    co2em(fe,"final",r))  ;                    // carbon emissions from final consumption

carblim_sec("energy",r)$deu(r)  = + sum(i,
                               + sum(fe,    co2em(fe,i,r)$ele(i))                                   // carbon emissions from electricity generation
                               + sum(fe,    co2em(fe,i,r)$(vom(i,r)$oil(i))));                      // carbon emisisons from oil refinery
...
*------18.09.2017
carblim_sec0(sec,"DEU") = carblim_sec(sec,"DEU");
...
* ------ 26.10.2017 R_supply_yr to check the development of the variable through the years
         r_supply_yr(*,*,yr)
...
A_pfad("2011","oil")=0.999923627878859;
A_pfad("2015","oil")=0.924545828580279;
A_pfad("2020","oil")=0.84376888729414;
A_pfad("2025","oil")=0.741271241485684;
A_pfad("2030","oil")=0.636205315966168;
A_pfad("2035","oil")=0.537293593630081;
A_pfad("2040","oil")=0.458182674239085;
A_pfad("2045","oil")=0.352842069849661;
A_pfad("2050","oil")=0.244559962258549;

A_pfad("2011","gas")=1.00085666400234;
A_pfad("2015","gas")=1.00892446476121;
A_pfad("2020","gas")=1.19894824784126;
A_pfad("2025","gas")=0.902807907990491;
A_pfad("2030","gas")=0.829843490319375;
A_pfad("2035","gas")=0.713220767919541;
A_pfad("2040","gas")=0.566748792115998;
A_pfad("2045","gas")=0.404393308903493;
A_pfad("2050","gas")=0.174462367137214;

A_pfad("2011","COL")=0.995222615391617;
A_pfad("2015","COL")=0.895439856157994;
A_pfad("2020","COL")=0.573466614439381;
A_pfad("2025","COL")=0.545880911053341;
A_pfad("2030","COL")=0.397033978528511;
A_pfad("2035","COL")=0.303768525153729;
A_pfad("2040","COL")=0.216073601217558;
A_pfad("2045","COL")=0.130008590010055;
A_pfad("2050","COL")=0.0655543314240886;
...
* ------ BEFORE-SOLVE ----------------------------------------------------------
*elast_e$after2015(yr) = 10;

* ------ update share of free allowances
gf_shr(i,r) = gf_shr_yr(yr,i,r);

elast_ene(r) = elast_ene_yr(r,yr);
* ------ scientific trick to make 2015 equals reality

vafm_input(i,gen,"deu")$yr2020(yr) = vafm_input0(i,gen);
skl_input(gen,"deu")$yr2020(yr) = skl_input0(gen);
usk_input(gen,"deu")$yr2020(yr) = usk_input0(gen);
cap_input(gen,"deu")$yr2020(yr) = cap_input0(gen);


vafm_input(i,"bBC","deu")$yr2015(yr) = 0.95 * vafm_input0(i,"bBC");
skl_input("bBC","deu")$yr2015(yr)    = 0.95 * skl_input0("bBC");
usk_input("bBC","deu")$yr2015(yr)    = 0.95 * usk_input0("bBC");
cap_input("bBC","deu")$yr2015(yr)    = 0.95 * cap_input0("bBC");

vafm_input(i,"mHC","deu")$yr2015(yr) = 0.9 * vafm_input0(i,"mHC");
skl_input("mHC","deu")$yr2015(yr)    = 0.9 * skl_input0("mHC");
usk_input("mHC","deu")$yr2015(yr)    = 0.9 * usk_input0("mHC");
cap_input("mHC","deu")$yr2015(yr)    = 0.9 * cap_input0("mHC");

vafm_input(i,"bGAS","deu")$yr2015(yr) = 0.97 * vafm_input0(i,"bGAS");
skl_input("bGAS","deu")$yr2015(yr)    = 0.97 * skl_input0("bGAS");
usk_input("bGAS","deu")$yr2015(yr)    = 0.97 * usk_input0("bGAS");
cap_input("bGAS","deu")$yr2015(yr)    = 0.97 * cap_input0("bGAS");
...
* ----- 14.03.2018 price collar test

*pco2_ets.up$(yr2025(yr)) =  35.6006822046455 * 0.9;
*pco2_ets.up$(yr2030(yr)) = 61.306080425606 * 0.9;
*pco2_ets.up$(yr2035(yr)) = 101.67933437261 * 0.9;
*pco2_ets.up$(yr2040(yr)) = 168.755568035907 * 0.9;
*pco2_ets.up$(yr2045(yr)) = 290.88593863295 * 0.9;
*pco2_ets.up$(yr2050(yr)) = 649.830391374329 * 0.9;
...
* ------ 24.01.2018 - changing switches for E3 scneario
eutrade$ ( (Enavi_E3 or Enavi_E4 or REEEM_all_tiCap_90 or REEEM_all_tiCap_95) and after2020(yr) ) = 1;
ETStrade$ ( (Enavi_E3 or Enavi_E4 or REEEM_all_tiCap_90 or REEEM_all_tiCap_95) and after2020(yr) ) = 0;
netstrade_r$ ( (Enavi_E3 or Enavi_E4 or REEEM_all_tiCap_90 or REEEM_all_tiCap_95) and after2020(yr) ) = 0;
...
*aeei_yr(r,e,g,"2011")$(deu(r) or fra(r) or aut(r) or eus(r))  = ep("2011",r) ;

* ------- 24.10.16 Same as two previous lines, but without aut(r)
...
******** TEST AEEI
*aeei_elexyr(r,gen,yr)$(india(r)) = 1;
*aeei_elenyr(r,gen,yr)$(india(r)) = 1;
* ###### 5.02.2016 Textmarke Comment out Diss (unten)
...
*aeei_elen("mGAS","DEU") = aeei_elexyr("DEU","mGAS",yr) ;
*aeei_elen("bBC","DEU")$yr2050(yr) = aeei_elexyr("DEU","bBC",yr);
*aeei_elen("bHC","DEU")$yr2050(yr) = aeei_elexyr("DEU","bHC",yr);
*aeei_elen("mHC","DEU")$yr2050(yr) = aeei_elexyr("DEU","mHC",yr);

* ----- 16.01.2018 making aeei_elex and aeei_elen equal for DEU and bBC, mHC and bHC (bbchere)
*aeei_elex(gen,r)$(deu(r) and (bBC(gen) or bHC(gen) or mHC(gen) ) and after2020(yr))= aeei_elenyr(r,gen,yr) * 0.2 ;
*aeei_elen(gen,r)$(deu(r) and (bBC(gen) or bHC(gen) or mHC(gen) ) and after2020(yr))= aeei_elenyr(r,gen,yr) * 0.5 ;
...
display gen_limit_yr, genlimit_twh_yr;
...
* ----- 18.01.2018
*pytarget("col","deu")$ ( pricetarget("col","deu") and yr2050(yr) and DEU_sec ) = 1.31;
*pytarget("gas","uki")$ ( pricetarget("gas","uki") and yr2050(yr) and DEU_sec ) = 1.55;

*pricetarget("col","deu")$yr2015(yr) = 0;
...
* ------ 04.01.2018 changed diffrebate to zero
...
ELEn.UP(gen,r)$(bnuc(gen) and fra(r) and after2040(yr) and REEEM_all_tiCap_95)  = 1.6 * elen.l(gen,r);
...
ELEn.UP(gen,r)$(bnuc(gen) and eus(r) and after2045(yr))  = elen.up(gen,r);

ELEn.UP(gen,r)$(bnuc(gen) and eus(r) and after2045(yr) and Enavi_D4 and Enavi_E3)  = 1.1 * elen.up(gen,r);
...
* ------ COAL, GAS and OIL
ELEn.UP(gen,r)$(mHC(gen)  and fra(r) and after2020(yr))  = 0;
ELEn.UP(gen,r)$(bbc(gen)  and ita(r)                  )  = 0;
ELEn.UP(gen,r)$(mhc(gen)  and ita(r) and after2030(yr)   )  = 0;

*ELEn.UP(gen,r)$(mgas(gen)  and uki(r) and after2035(yr)   )  = 1.1 * elen.l(gen,r);

*ELEn.UP(gen,r)$(bgas(gen)  and ita(r) and after2025(yr)   )  = 1.05 * elen.up(gen,r);

ELEn.UP(gen,r)$(bbc(gen)  and eun(r) and yr2030(yr))  = 0;
*ELEn.UP(gen,r)$(bhc(gen)  and eun(r) and yr2015(yr))  = 0;
*ELEn.UP(gen,r)$(mhc(gen)  and eun(r) and yr2015(yr))  = 0;
ELEx.fx(gen,r)$(bbc(gen)  and eus(r) and yr2030(yr))  = elen.up(gen,r);
ELEn.up(gen,r)$(bbc(gen)  and eus(r) and after2030(yr))  = elen.l(gen,r);

*ELEn.UP(gen,r)$(pOIL(gen)  and deu(r) and yr2050(yr) and noCOAL_DEU)  =  elen.l(gen,r) ;

*ELEn.UP(gen,r)$(( mHC(gen) or bHC(gen) ) and deu(r) and after2020(yr))  = 0;
...
*ELEn.UP(gen,r)$(bhydro(gen) and eus(r) and after2015(yr))  = 0.95;
*ELEn.UP(gen,r)$(bhydro(gen) and eus(r) and after2025(yr))  = 1.1;
*ELEn.UP(gen,r)$(bhydro(gen) and eus(r) and after2035(yr))  = 1.3;
...
* ------ mWIND and mSOLAR
*ELEx.up(gen,r)$(mwind(gen)  and esp(r)                  )= 0.985 *  abschreibung_bmk(r,gen,yr) ;

*ELEn.up(gen,r)$(mwind(gen)  and esp(r) and after2040(yr))= 1.1 * elen.l(gen,r) ;

*ELEn.up(gen,r)$(mwind(gen)  and pol(r) and yr2030(yr) and ((Enavi_E3 and REEEM_run) or Enavi_E4))= 2 * elen.l(gen,r) ;
*ELEn.up(gen,r)$(mwind(gen)  and pol(r) and after2030(yr))= 1.5 * elen.l(gen,r) ;
*ELEn.up(gen,r)$(mwind(gen)  and pol(r) and after2040(yr))= 1.1 * elen.l(gen,r) ;

*ELEn.up(gen,r)$(mwind(gen)  and pol(r) and after2045(yr) and Enavi_E3 and Enavi_D4)= 1.2 * elen.l(gen,r) ;

ELEn.fx(gen,r)$(mwind(gen)  and deu(r) and yr2015(yr))= 0.78 ; //

*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and yr2020(yr))= 2.2 * ELEn.L(gen,r) ;
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and yr2025(yr)  and (DEU_sec or Enavi_E3 or Enavi_D4 or Enavi_E4 or REEEM_all_tiCap_95 or REEEM_all_tiCap_90))= 1.54 * ELEn.L(gen,r) ; 
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and yr2030(yr)  and (Enavi_E3 or noCOAL_DEU or Enavi_E4 or Enavi_D4 or REEEM_all_tiCap_95 or REEEM_all_tiCap_90))= 1.2 * ELEn.L(gen,r) ; 
ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2020(yr))= 1.2 * ELEn.L(gen,r) ; // BASELINE
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2030(yr) and (DEU_sec or Enavi_E4))= 1.18 * ELEn.L(gen,r) ; //
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2030(yr)  and (noCOAL_DEU))= 1.28 * ELEn.L(gen,r) ; 

*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2030(yr) and (Enavi_E3  or REEEM_all_tiCap_90))= 1.25 * ELEn.L(gen,r) ; //
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2040(yr) and (Enavi_E3  or REEEM_all_tiCap_90))= 1.2 * ELEn.L(gen,r) ; //

*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2030(yr) and (Enavi_D4))= 1.21 * ELEn.L(gen,r) ; //
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2040(yr) and (Enavi_D4))= 1.1 * ELEn.L(gen,r) ; //

*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2030(yr) and ((Enavi_D4 and Enavi_E4) or REEEM_all_tiCap_95))= 1.3 * ELEn.L(gen,r) ; //
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2040(yr) and ((Enavi_D4 and Enavi_E4)or REEEM_all_tiCap_95))= 1.2 * ELEn.L(gen,r) ; //
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and yr2050(yr) and (Enavi_D4 and Enavi_E4))= 1.05 * ELEn.L(gen,r) ;

*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2030(yr) and (Enavi_E3 and Enavi_D4))= 1.25 * ELEn.L(gen,r) ; // 
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2040(yr) and (Enavi_E3 and Enavi_D4))= 1.2 * ELEn.L(gen,r) ; // 

*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2040(yr) and (noCOAL_DEU or Enavi_D2))= 1.3* ELEn.L(gen,r) ; //
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and yr2050(yr) and (noCOAL_DEU and  Enavi_E2k))= 1.4* ELEn.L(gen,r) ; //
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and yr2050(yr) and (noCOAL_DEU or Enavi_D2))= 1.05* ELEn.L(gen,r) ; // 

*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2035(yr) and Enavi_E2 and Enavi_D2)= 1.21* ELEn.L(gen,r) ; // 
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2045(yr) and Enavi_E2 and Enavi_D2)= 1.3* ELEn.L(gen,r) ; // 
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and yr2050(yr) and Enavi_E2 and Enavi_D2)= 1.1* ELEn.L(gen,r) ; //

*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2030(yr) and Enavi_E3 and Enavi_D2)= 1.2* ELEn.L(gen,r) ; // 
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2030(yr) and Enavi_E3 and Enavi_D2)= 1.3* ELEn.L(gen,r) ; //
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and yr2050(yr) and Enavi_E3 and Enavi_D2)= 1.1* ELEn.L(gen,r) ; //
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and yr2050(yr) and Enavi_E3 and Enavi_D2)= 1.1* ELEn.L(gen,r) ; //


ELEn.fx(gen,r)$(msolar(gen)  and deu(r) and yr2015(yr))= 1 ; //

*ELEn.up(gen,r)$(msolar(gen)  and deu(r) and yr2020(yr))= 2 * elen.l(gen,r) ; // 
*ELEn.up(gen,r)$(msolar(gen)  and deu(r) and yr2025(yr))= 2 * elen.l(gen,r) ; // 
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and yr2030(yr) and (Enavi_E3 or Enavi_E4  or REEEM_all_tiCap_95 or REEEM_all_tiCap_90))= 1.7 * ELEn.L(gen,r);
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2030(yr) and not DEU_sec)= 1.14 * ELEn.L(gen,r) ;
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2030(yr) and deu_sec)= 1.35 * ELEn.L(gen,r) ; 

ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2020(yr))= 1.2 * ELEn.L(gen,r) ;

*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2030(yr) and (Enavi_E3 or Enavi_E4))= 1.19 * ELEn.L(gen,r) ; 
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and yr2050(yr) and (Enavi_E3 or Enavi_E4))= 1.05 * ELEn.L(gen,r) ; 

*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2030(yr) and (Enavi_E3 or Enavi_E4))= 1.19 * ELEn.L(gen,r) ; 
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and yr2050(yr) and (Enavi_E3 or Enavi_E4))= 1.05 * ELEn.L(gen,r) ; 

*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2030(yr) and (Enavi_E4))= 1.15 * ELEn.L(gen,r) ; 
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and yr2050(yr) and (Enavi_E4))= 1.04 * ELEn.L(gen,r) ; 

*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2030(yr) and Enavi_E3 and Enavi_D4)= 1.25 * ELEn.L(gen,r) ; 
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2040(yr) and Enavi_E3 and Enavi_D4)= 1.14 * ELEn.L(gen,r) ; 

*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2030(yr) and (Enavi_D2))= 1.6 * ELEn.L(gen,r) ; //base
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and yr2050(yr) and (Enavi_D2))= 1.4 * ELEn.L(gen,r) ; //base

*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2030(yr) and (Enavi_D2 and enavi_e3))= 1.1 * ELEn.L(gen,r) ; //base
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2030(yr) and (Enavi_D2 and enavi_e3))= 1.2 * ELEn.L(gen,r) ; //base
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and yr2050(yr) and (Enavi_D2 and enavi_e3))= 1.06 * ELEn.L(gen,r) ; //base

*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2030(yr) and (noCOAL_DEU))= 1.33 * ELEn.L(gen,r) ; //base
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and yr2050(yr) and (noCOAL_DEU))= 1.2 * ELEn.L(gen,r) ; //base
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and yr2050(yr) and (noCOAL_DEU and Enavi_E2k))= 1.3 * ELEn.L(gen,r) ; //base

*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2030(yr) and (Enavi_D2 and Enavi_E2))= 1.53 * ELEn.L(gen,r) ; //base
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2045(yr) and (Enavi_D2 and Enavi_E2))= 1.8 * ELEn.L(gen,r) ; //base
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and yr2050(yr) and Enavi_E2 and Enavi_D2)= 1.4 * ELEn.L(gen,r) ; // 

*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and after2030(yr) and ((REEEM_run and enavi_e3) or REEEM_all_tiCap_95 or REEEM_all_tiCap_90))= 1.12 * ELEn.L(gen,r) ; //base
*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and yr2050(yr) and ((REEEM_run and enavi_e3) or REEEM_all_tiCap_90))= 1.04 * ELEn.L(gen,r) ; //base 

*ELEn.UP(gen,r)$(msolar(gen)  and deu(r) and yr2050(yr) and REEEM_all_tiCap_95)= 1.4 * ELEn.L(gen,r) ; //base

* ------ 16.02.2015 CCS Limitations
ELEn.UP(gen,r)$(fosccs(gen)              and before2030(yr))     = ELEn.L(gen,r) ;
ELEn.UP(gen,r)$(mccs(gen)                and after2025(yr) and not DEU(r))      = ELEn.L(gen,r) + 750 * (1-thetax);
ELEn.UP(gen,r)$(bccs(gen)                and after2025(yr) and not DEU(r))      = ELEn.L(gen,r) + 750 * (1-thetax);

*----- 03.03.2017 Switch to turn germany's CCS on and off
ELEn.UP(gen,r)$(mccs(gen)                and after2025(yr) and DEU(r) and DEU_CCS)      = ELEn.L(gen,r) + 750 * (1-thetax);
ELEn.UP(gen,r)$(bccs(gen)                and after2025(yr) and DEU(r) and DEU_CCS)      = ELEn.L(gen,r) + 750 * (1-thetax);

ELEn.UP(gen,r)$(mccs(gen)                and after2025(yr) and DEU(r) and not DEU_CCS)      = ELEn.L(gen,r);
ELEn.UP(gen,r)$(bccs(gen)                and after2025(yr) and DEU(r) and not DEU_CCS)      = ELEn.L(gen,r);

*------ 10.12.2014 RWE Szenario für bBC-Zubau-Beschränkung
*ELEn.UP(gen,r)$(bbc(gen) and deu(r))     = 1.0 * bbc_deu_elen(gen,yr) * (1-thetax);  // REF
*ELEn.UP(gen,r)$(bbc(gen) and deu(r) and before2035(yr))     = 1.0 * bbc_deu_elen(gen,yr) * (1-thetax);  // REF
...
*------ 03.03.2017 decomissioning coal in germany till 2030
...
co2pfad_ets(r,yr)$(eu28(r) and notrade)  = co2pfad_ets_eu("EU28",yr);
...
co2pfad_ets(r,yr)$(not eu28(r) and row_notrade and REEEM_regpush and after2020(yr))  = co2pfad_regpush(r,yr); // 2 degree pathway (from 2020 onwards)
...
* ----- 05.03.2018 read the ets co2 growth path for the tighter cao scenarios 
...
* ----- 26.01.2018 for D4 enavi scenario
...
*co2pfad_nets(r,yr)$(eu28(r) and netstrade_r and scenario_2dc)= co2pfad_nonets_2dc(r,yr) ;                   display co2pfad_nets;
...
display co2pfad_ets, co2pfad_ets_eu, co2pfad_nets, carblim_ets, carblim_ets0, carblim0, carblim, co2pfad_ets_row;
...
eff_target(r)$(deu(r) and yr2011(yr))    = eff_avg(r) ;
...
ssd(r,ct,cf)$after2015(yr)= ssd(r,ct,cf) * (1 - anz_car_newrel(r,yr)/100)**5 + anz_car(r,ct,cf,"new",yr) * ssd0price(r,ct,cf) * 5; display ssd;
...
* ------ BUILDING STOCK DYNAMICS
bsd(r,bt,bf)$(yr2011(yr) and old(bt)) = bsd(r,bt,bf) * (1 - 0.8 * anz_qm_newrel(r,yr)/100)**3 ;
...
*bsd(r,bt,bf)$(yr2011(yr) and old(bt)) = bsd(r,bt,bf) * (1 - 1.552/100)**3 ;
...
* ------ 8.02.2016 Updating
bsdnh(r,bt,bf)$yr2011(yr) = bsdnh(r,bt,bf) + anz_qm_new(r,bt,bf,yr) * qmcostc_disc(r,bt,bf) * 3;
...
carblim_sec_yr(sec,"DEU",yr) = carblim_sec(sec,"DEU");
