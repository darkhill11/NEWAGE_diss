...
* ------ Quotas/Flags/Shares in electricty generation and ETS allowances
         ...
         etstrade        Defines carbon regime as EU-28 ETS      permit price PCO2_ETS                   / 1 /   // ETStrade --> Schalter for basis scenario
...
*         aeei_variante   Schalter f�r Einstaz der McKinsey AEEI sektoral u. zeitlich spezifiziert        / 0 /
         ...
         REEEM_cluni      switch for turning on Cluster Union scenario for REEEM                            / 1 /
         REEEM_run        switch just to make sure it is a reeem Scenario (if Schalter enavi_e3 is active)  / 0 /
         REEEM_regpush    switch to turn on the regional push scenario for REEEM                            / 1 /
...
carblim_sec("buildings",r)$deu(r) = carblim_sec("buildings",r) +  sum(fe,    co2em(fe,"final",r))  ;                    // carbon emissions from final consumption
...
ELEn.up(gen,r)$(ele_dev_switch(gen,r) AND times_coupling AND NOT (mWIND(gen) OR msolar(gen) ) ) = 1.08 * (ele_dev(gen,r,yr)$((ele_dev(gen,r,yr) - abschr_pfad(r,gen,yr)) gt 0) - abschr_pfad(r,gen,yr)$((ele_dev(gen,r,yr) - abschr_pfad(r,gen,yr)) gt 0) + 0$((ele_dev(gen,r,yr) - abschr_pfad(r,gen,yr)) le 0));
...
ELEn.lo(gen,r)$(ele_dev_switch(gen,r) AND times_coupling AND NOT (mWIND(gen) OR msolar(gen) ) ) = 0.92 * (ele_dev(gen,r,yr)$((ele_dev(gen,r,yr) - abschr_pfad(r,gen,yr)) gt 0) - abschr_pfad(r,gen,yr)$((ele_dev(gen,r,yr) - abschr_pfad(r,gen,yr)) gt 0) + 0$((ele_dev(gen,r,yr) - abschr_pfad(r,gen,yr)) le 0));
ELEn.lo(gen,r)$(ele_dev_switch(gen,r) AND times_coupling AND (msolar(gen) OR mwind(gen))) = 0.98 * (ele_dev(gen,r,yr)$((ele_dev(gen,r,yr) - abschr_pfad(r,gen,yr)) gt 0) - abschr_pfad(r,gen,yr)$((ele_dev(gen,r,yr) - abschr_pfad(r,gen,yr)) gt 0) + 0$((ele_dev(gen,r,yr) - abschr_pfad(r,gen,yr)) le 0));
