...
EU28(r)$detrade =EU28_de(r);
EU28(r)$(not detrade) =EU28_eu(r);
...
* ------ LOAD
$LOAD    ressize
$LOAD    co2pfad_ets  co2pfad co2pfad_ets_eu co2pfad_ets_row co2pfad_ets_row_2dc co2pfad_nonets co2pfad_nonets_2dc
$LOAD    URUN0  URSK0
...
* ------ Quotas/Flags/Shares in electricty generation and ETS allowances
         ...
         eutrade         Defines carbon regime as international  permit price PCO2W for EU28             / 1 /   // EUtrade
         etstrade        Defines carbon regime as EU-28 ETS      permit price PCO2_ETS                   / 0 /   // ETStrade
...
notrad(r)$(not eu28(r) and row_notrade) = yes;
...
* XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
* ------ 06.11.2017
carbon_tax = carbon_tax_yr("DEU",yr);

* 25.10.2017 ------- trick to work with exo prices
*pricetarget("cru",r)             = 1 ;   // BAU on
*pricetarget("col",r)             = 1 ;   // BAU on
*pricetarget("gas",r)             = 1 ;   // BAU on

*pricetarget("cru",r)$(YR2015(YR))              = 0 ;   // BAU on
*pricetarget("col",r)$(YR2015(YR))              = 0 ;   // BAU on
*pricetarget("gas",r)$(YR2015(YR))              = 0 ;   // BAU on

R_SUPPLY.l("gas","IND")$(YR2015(yr)) = 2.27466;
R_SUPPLY.l("gas","UKI")$(YR2015(yr)) = 1.80182;
R_SUPPLY.l("gas","EUS")$(YR2015(yr)) = 0.80989;

* --------- 07.11.2017 reduce renewables costs
*diffcost(gen,r)$(cost_red(gen,yr) and deu(r)) = diffcost(gen,r) * cost_red(gen,yr); 
...
* ------ COAL, GAS and OIL
ELEn.UP(gen,r)$(moil(gen)            and after2015(yr))  = 2.0 * ELEn.L(gen,r) + (1-thetax);
ELEn.UP(gen,r)$(bGAS(gen) and eu28(r) and after2015(yr)) = 4 - ELEx.L(gen,r); // commented
ELEn.UP(gen,r)$(bGAS(gen) and deu(r) and after2015(yr))  = 3 - ELEx.L(gen,r); // commented
*ELEn.UP(gen,r)$(bGAS(gen) and eu28(r) and after2015(yr)) = 2 - ELEx.L(gen,r); // commented
*ELEn.UP(gen,r)$(bGAS(gen) and deu(r) and after2015(yr))  = 1.5 - ELEx.L(gen,r); // commented
*ELEn.UP(gen,r)$(bgas(gen) and arb(r) and after2035(yr))  = 1.2 * ELEn.L(gen,r) + (1-thetax);
ELEn.UP(gen,r)$(mgas(gen) and deu(r) and after2025(yr))  = 2 * ELEn.L(gen,r) ; // changed here
...
* ------ mWIND and mSOLAR
ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and yr2020(yr))= 2 * ELEn.L(gen,r) ; // changed here
ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2020(yr))= 1.3 * ELEn.L(gen,r) ; // changed here
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2015(yr))= 1.3 * ELEn.L(gen,r) ; // changed here
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2015(yr) and before2035(yr))= 1.5 * ELEn.L(gen,r) ;
*ELEn.UP(gen,r)$(mwind(gen)  and deu(r) and after2030(yr))= 1.2 * ELEn.L(gen,r) ;

ELEn.UP(gen,r)$(mwind(gen)  and eus(r) and after2015(yr))= 1.5 * ELEn.L(gen,r) ;
...
ELEn.UP(gen,r)$(msolar(gen) and deu(r) and after2025(yr))= 1.55 * ELEn.L(gen,r) ; // changed here
ELEn.UP(gen,r)$(msolar(gen) and deu(r) and after2045(yr))= 1.8 * ELEn.L(gen,r) ; // changed here
*ELEn.UP(gen,r)$(msolar(gen) and deu(r) and after2015(yr))= 1.35 * ELEn.L(gen,r) ; // changed here
